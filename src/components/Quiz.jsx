...
//
// wrongCounts    – shared miss map from App (keyed by `item.char`)
// onRegisterMiss – callback to App that increments the miss counter,
//                  which increases the probability this character reappears.
export default function Quiz({ mode, wrongCounts, onRegisterMiss }) {
  ...
  const handleSelect = (opt) => {
    ...
    if (!isCorrect) {
      // Notify App to increase the miss weight for this character.
      // Higher weight = more likely to be drawn again in future questions.
      onRegisterMiss(question.correct.char);
    }
  };
